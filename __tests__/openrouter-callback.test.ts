...
describe('GET /api/openrouter/callback', () => {
  ...
  it('exchanges code for API key and saves to config', async () => {
    ...
    const res = await GET(getReq('http://localhost/api/openrouter/callback?code=auth-code-456'));
    expect(res.status).toBe(307);
  ...
  it('passes code_verifier to OpenRouter when provided', async () => {
    ...
    await GET(getReq('http://localhost/api/openrouter/callback?code=abc&code_verifier=verifier123'));
  ...
  it('redirects with error when exchange returns non-ok', async () => {
    ...
    const res = await GET(getReq('http://localhost/api/openrouter/callback?code=bad'));
    expect(res.status).toBe(307);
  ...
  it('redirects with error when response has no key', async () => {
    ...
    const res = await GET(getReq('http://localhost/api/openrouter/callback?code=no-key'));
  ...
  it('redirects with error on network failure', async () => {
    ...
    const res = await GET(getReq('http://localhost/api/openrouter/callback?code=net-err'));
  ...
  it('handles malformed JSON from exchange gracefully', async () => {
    ...
    const res = await GET(getReq('http://localhost/api/openrouter/callback?code=bad-json'));
...
describe('CSRF state validation', () => {
  ...
  it('allows request when state values match', async () => {
    ...
    const res = await GET(getReq(
      `http://localhost/api/openrouter/callback?code=good&state=${state}`,
      { openrouter_oauth_state: state },
    ));
    expect(new URL(res.headers.get('location')!).searchParams.get('openrouter')).toBe('connected');
    expect(writeConfigMock).toHaveBeenCalled();
  });

  it('allows legacy requests without any state (backwards compat)', async () => {
    fetchSpy.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ key: 'sk-or-legacy' }),
    });

    // No state in URL, no cookie — both absent, so the check is skipped
