...
### Tests
- Unit/integration coverage expanded again: `__tests__/yts-popular.test.ts` now covers multi-page filtered pagination and exact end-of-scan totals, `__tests__/media-keys.test.ts` covers movie-vs-TV torrent-key separation, and `__tests__/recUrlParam.test.ts` covers `strictYear` preservation through the `?rec=` handoff. `__e2e__/app-smoke.test.ts` boots the built standalone app and smoke-tests core routes over real HTTP. `__tests__/rate-limit.test.ts` covers the new shared limiter including the soft-cap eviction path. 35 test files / 632 unit+integration tests at this commit
