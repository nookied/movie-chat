...
- **Explicit Node runtime policy** (`.nvmrc`, `.node-version`, `package.json`, `scripts/check-node-version.mjs`, `.github/workflows/test.yml`): The repo now pins local development to Node 24 LTS, keeps Node 20 LTS supported, fast-fails unsupported local runtimes before build/dev/start/CI entrypoints, and verifies both majors in CI instead of implicitly following unsupported current releases
...
- **Install/setup/update paths could run on unsupported current Node majors** (`install.sh`, `setup.sh`, `update.sh`): These scripts now require tested LTS majors only (Node 20 or 24). `update.sh` checks Node before any git/npm work so unsupported runtimes fail fast instead of reaching a stuck `next build`
...
### Tests
- Unit/integration coverage expanded again: `__tests__/yts-popular.test.ts` now covers multi-page filtered pagination and exact end-of-scan totals, `__tests__/media-keys.test.ts` covers movie-vs-TV torrent-key separation, and `__tests__/recUrlParam.test.ts` covers `strictYear` preservation through the `?rec=` handoff. `__e2e__/app-smoke.test.ts` boots the built standalone app and smoke-tests core routes over real HTTP. Final counts verified in `HANDOFF.md` after the latest run.
- `__tests__/shell-scripts.test.ts` now also verifies that `update.sh --auto` fails fast on unsupported Node versions before any git/npm work begins
