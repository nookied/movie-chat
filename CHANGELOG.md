...
### Fixed
- **Text wrapping clip in PopularMovieCard** (`components/PopularMovieCard.tsx`): Replaced block layout with flexbox and set a strict `min-h-[2.5rem]` on the title to prevent WebKit grid intrinsic height bugs from horizontally cropping titles that wrap to a second line.
