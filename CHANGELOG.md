...
### Fixed
- **YTS singular/plural title mismatch** (`lib/yts.ts`): Added `isPrefixMatch()` as a final candidate in `queryYts` — when a normalized title is a prefix of the YTS entry (or vice versa), differs by ≤ 2 chars, and both are ≥ 8 chars long, the match is accepted. Fixes cases where TMDB and YTS disagree on singular vs plural (e.g. "Forbidden Fruit" vs YTS "Forbidden Fruits") causing available 1080p torrents to go undetected.
- **Popular movie card title wrapping** (`components/PopularMovieCard.tsx`): Long titles now use a fixed-height two-line title slot with shrink-safe card/text containers, keeping descenders visible while preventing mobile grid blowouts.
- **Popular Movies control wrapping** (`components/PopularMoviesPanel.tsx`): Tab and filter controls now use shrink-safe mobile layouts, with the genre/year selects moving as a stable two-column group on narrow screens.
...
### Simplify pass
- **Popular movies panel layout cleanup** (`components/PopularMoviesPanel.tsx`): Split the tab selector, newest sort select, downloaded filters, result counter, and loading skeleton into small named helpers, and centralised the YTS query-param construction so the component is easier to review without changing the browse flow.
